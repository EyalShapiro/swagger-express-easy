...
describe('Advanced Swagger Features', () => {
  ...
  test('withSwagger should register route correctly', () => {
    const handler = (req: Request, res: Response) => res.json({ ok: true });

    const wrapped = withSwagger(
      {
        method: 'post',
        path: '/api/typed',
  ...
  test('withTypedSwagger should work with explicit generics', () => {
    ...
    const handler = (req: Request<any, any, MyBody>, res: Response) =>
    ...
    withSwagger({ method: 'get', path: '/api/explicit' }, handler as any);
