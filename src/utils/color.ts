...
const ERROR = { bg: 'node-color-log warning: Background color not found! Use the default.' };
class Logger {
  command: string = '';
  lastCommand: string = '';
  name: string = '';
  ...
  noColor: boolean;
  _getDate: () => string;
  _customizedConsole: Console;
  _enableFileAndLine: { enable: boolean; isShortFile: boolean };
  ...
  createNamedLogger(name: string | undefined) {
  ...
  setLogStream(newStream: NodeJS.WritableStream) {
  ...
  isLevelValid(level: string | undefined) {
  ...
  enableFileAndLine(enable: boolean, isShortFile = false) {
  ...
  color(ticket: string) {
    if (ticket in CONFIG.FONT) {
      this.command += CONFIG.FONT[ticket as keyof typeof CONFIG.FONT];
    } else {
  ...
  bgColor(ticket: string) {
    if (ticket in CONFIG.BACKGROUND) {
      this.command += CONFIG.BACKGROUND[ticket as keyof typeof CONFIG.BACKGROUND];
    } else {
      console.error(ERROR.bg);
    }
    return this;
  }
  ...
  fontColorLog(ticket: string, text: string, setting: any) {
    ...
    if (ticket in CONFIG.FONT) {
      command += CONFIG.FONT[ticket as keyof typeof CONFIG.FONT];
    } else {
      console.error('node-color-log warning: Font color not found! Use the default.');
    }
    command += text;
  ...
  bgColorLog(ticket: string, text: string, setting: any) {
    ...
    if (ticket in CONFIG.BACKGROUND) {
      command += CONFIG.BACKGROUND[ticket as keyof typeof CONFIG.BACKGROUND];
    } else {
      console.error(ERROR.bg);
    }
    command += text;
  ...
  colorLog(ticketObj: { font: string; bg: string }, text: string, setting: any) {
    ...
    const { font, bg } = ticketObj;
    if (font in CONFIG.FONT) {
      command += CONFIG.FONT[font as keyof typeof CONFIG.FONT];
    } else {
      console.error('node-color-log warning: Font color not found! Use the default.');
    }
    if (bg in CONFIG.BACKGROUND) {
      command += CONFIG.BACKGROUND[ticketObj.bg as keyof typeof CONFIG.BACKGROUND];
    } else {
      console.error(ERROR.bg);
    }
  ...
  checkSetting(setting: { [x: string]: boolean }) {
    ...
    for (const item in setting) {
      if (validSetting.indexOf(item) !== -1) {
        if (setting[item] === true) {
          command += CONFIG.SYSTEM[item as keyof typeof CONFIG.SYSTEM];
  ...
  append(...args: (number | string)[]) {
    for (const idx in args) {
      ...
      if (args.length > 1 && Number(idx) < args?.length - 1) {
        this.command += ' ';
      }
    }
    return this;
